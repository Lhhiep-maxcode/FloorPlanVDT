...
class Room():
    ...
    def rooms_from_data(data):
        ...
        for i in range(len(data['types'])):
            room = Room()
            room.box = np.array(data['boxes_aligned'][i],dtype=int)
            room.category = np.array(data['types'][i],dtype=int)
            room.boundary = np.array(data['room_boundaries'][i],dtype=float)
    ...
    def from_node_box(node,box):
        x0,y0,x1,y1 = box
        room = Room()
        room.box = np.array(box,dtype=int)
    ...
    def from_boundary(boundary):
        ...
        room.category = 0
        room.box = np.array([np.min(boundary[:,0]),np.min(boundary[:,1]),np.max(boundary[:,0]),np.max(boundary[:,1])],dtype=int)
        room.boundary = np.array(boundary[:,:2],dtype=int)
        room.lines = DirectedLine.lines_from_boundary(room.boundary)
        # room.boundary = [
...
def rooms_to_numpy(rooms):
    ...
    for i in range(len(rooms)):
        ...
        if len(rooms[i].windows) > 0:
            ...
            windows.extend(ws)
    return np.array(doors,dtype=int),np.array(windows,dtype=int)

def add_door_window(data):
    boundary = data['boundary']
    living_idx = data['types'].tolist().index(0)
